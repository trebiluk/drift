...
void main() {
  ...
  vec3 zenith = mix(vec3(0.02, 0.22, 0.68), vec3(0.02, 0.04, 0.12), uNight);
  vec3 horizon = mix(vec3(0.42, 0.68, 0.94), vec3(0.14, 0.18, 0.34), uNight);
  vec3 below = mix(vec3(0.28, 0.52, 0.86), vec3(0.06, 0.08, 0.16), uNight);
...
void main() {
  ...
  vec3 valley = mix(vec3(0.78, 0.86, 0.94), vec3(0.10, 0.14, 0.24), uNight);
  vec3 peak = mix(vec3(1.0, 1.0, 0.99), vec3(0.58, 0.64, 0.80), uNight);
  ...
  vec3 fogCol = mix(vec3(0.55, 0.74, 0.92), vec3(0.08, 0.11, 0.2), uNight);
...
void main() {
  ...
  vLight = 0.86 + sunSide * 0.22 + uv.y * 0.12;
...
void main() {
  float radial = length(vLocal - vec2(0.5)) * 2.0;
  if (radial > 0.86) discard;
  vec4 t = texture2D(uMap, vUv);
  float a = t.a * (1.0 - smoothstep(0.48, 0.86, radial));
  if (a < 0.08) discard;
  vec3 view = normalize(vView);
  vec3 sun = normalize(uSun);
  float backlit = pow(max(dot(view, sun), 0.0), 3.5);
  vec3 fluff = mix(vec3(0.99, 0.995, 1.0), t.rgb, 0.28);
  vec3 col = fluff * mix(vLight, vLight * 0.55 + 0.18, uNight);
  col += mix(vec3(1.0, 0.97, 0.9), vec3(0.7, 0.82, 1.0), uNight) * backlit * mix(0.38, 0.22, uNight);
  float fade = smoothstep(980.0, 160.0, vDist) * smoothstep(36.0, 110.0, vDist);
  float spaceFade = 1.0 - uSpace * 0.78;
  float alpha = a * fade * spaceFade;
